...
module.exports = class extends Command {
	...
	async run(message, [target, ...args]) {
		...
		if (member.id === message.author.id) return message.quote('You can\'t warn yourself!');
		const memberPosition = member.roles.highest.position;
		const moderationPosition = message.member.roles.highest.position;
		if (message.member.ownerID !== message.author.id && !(moderationPosition > memberPosition)) {
			return message.quote('You can\'t warn a member who has an higher or equal role hierarchy to yours!');
		}
		...
		if (banCount) {
			...
			message.quote(`**${member.user.tag}** has been warned in private messages for **${reason}**!`);
		}
